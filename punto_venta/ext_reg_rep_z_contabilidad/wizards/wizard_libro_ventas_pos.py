...
class LibroVentasPosModelo(models.Model):
    ...
    def formato_fecha2(self,date):
        ...
        if hora>=0 and hora<=7:
            dia=dia-1
        #raise UserError(_('Hora: %s')%hora)
        resultado=str(dia)+"/"+mes+"/"+ano
        return resultado
...
class libro_ventas(models.TransientModel):
    ...
    def get_invoice(self):
        ...
        for det in cursor_resumen:
            ...
            'sale_total': self.conv_div_bs(det.total_con_iva-det.total_con_iva_nc,det),
            'base_imponible': self.conv_div_bs(det.total_base,det),
            'iva' : self.conv_div_bs(det.total_valor_iva-det.total_valor_iva_nc,det),
            'total_exento':self.conv_div_bs(det.total_exento-det.total_exento_nc,det),
            'alicuota_reducida':self.conv_div_bs(det.alicuota_reducida-det.alicuota_reducida_nc,det),
            'alicuota_general':self.conv_div_bs(det.alicuota_general-det.alicuota_general_nc,det),
            'alicuota_adicional':self.conv_div_bs(det.alicuota_adicional-det.alicuota_adicional_nc,det),
            'base_adicional':self.conv_div_bs(det.base_adicional-det.base_adicional_nc,det),
            'base_reducida':self.conv_div_bs(det.base_reducida-det.base_reducida_nc,det),
            'base_general':self.conv_div_bs(det.base_general-det.base_general_nc,det),
...
# *******************  REPORTE EN EXCEL ****************************
    def generate_xls_report(self):
        ...
        ws1.write_merge(row, row, 2, 4,'J-'+ str(self.company_id.partner_id.vat), sub_header_content_style)
        row+=1
